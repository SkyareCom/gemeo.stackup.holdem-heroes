...
CASES = [
    ...
    dict(id="cash-btn-vs-bb-river-value", street="river", pot_type="SRP", texture="Q-high-rainbow", board="Qh,7d,4s,2c,6c", pot=36, stack=50, hero="QsJs", path=["CHECK"], expected_app="BET"),
    dict(id="cash-bb-vs-btn-river-bet", street="river", pot_type="SRP", texture="K-high-rainbow", board="Kc,9d,5s,4h,2d", pot=90, stack=68, hero="KhQh", path=["CHECK", "BET"], expected_app="CALL"),
    dict(id="mtt-btn-vs-bb-turn-chipEV", street="turn", pot_type="SRP", texture="A-overcard-rainbow", board="Qd,8s,4c,Ah", pot=15, stack=33, hero="KsQc", path=["CHECK"], expected_app="CHECK"),
    dict(id="cash-a72r-oop-vs-bet", street="flop", pot_type="SRP", texture="A-high-dry-rainbow", board="Ah,7d,2c", pot=10, stack=95, hero="AsJs", path=["CHECK", "BET"], expected_app="CALL"),
    dict(id="cash-q77r-ip-after-check", street="flop", pot_type="SRP", texture="paired-rainbow", board="Qh,7d,7c", pot=12, stack=94, hero="QsJs", path=["CHECK"], expected_app="BET"),
    dict(id="cash-t98tt-oop-vs-bet", street="flop", pot_type="SRP", texture="connected-two-tone", board="Th,9h,8c", pot=12, stack=94, hero="JsTs", path=["CHECK", "BET"], expected_app="CALL"),
    dict(id="cash-ac9c4c-ip-after-check", street="flop", pot_type="SRP", texture="monotone", board="Ac,9c,4c", pot=14, stack=93, hero="KcQs", path=["CHECK"], expected_app="BET"),
    dict(id="cash-low-connected-oop-vs-bet", street="flop", pot_type="SRP", texture="low-connected-rainbow", board="8d,7s,6c", pot=11, stack=95, hero="9h8h", path=["CHECK", "BET"], expected_app="CALL"),
...
def choose_child(node, prefix):
    ...
    return sorted(candidates, key=lambda kv: kv[0])[0][1]
