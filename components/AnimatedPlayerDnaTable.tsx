...
const fallbackPositions=["UTG","UTG+1","MP","MP+1","HJ","CO","BTN","SB","BB","LJ"];
...
export default function AnimatedPlayerDnaTable({spot,selectedAction,onSequenceReady}:Props){
  ...
  const ringSeats:Seat[]=fallbackPositions.map(position=>{
  ...
  opponents.filter(player=>!used.has(player.position)).forEach((player,index)=>{const slot=ringSeats.findIndex(seat=>seat.placeholder);if(slot>=0)ringSeats[slot]=player;else if(index<ringSeats.length)ringSeats[index]=player});
    ...
    <div className={styles.table}>
      <div className={styles.felt}>
        ...
        <div className={`${styles.heroSeat} ${ready?styles.heroReady:""}`}>
          ...
          <div className={styles.heroPlate}><strong>{hero.position} · HERO</strong><span>{fmt(hero.stack,spot.mode)}</span></div>
