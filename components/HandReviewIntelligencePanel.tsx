...
const REVIEW_LIBRARY_KEY="stackup.hand-review.library.v1";
...
export default function HandReviewIntelligencePanel({input}:{input:HandReviewInput}){
 ...
 function saveEvolution(){
  const raw=localStorage.getItem(REVIEW_LIBRARY_KEY);let rows:unknown[]=[];try{const parsed=raw?JSON.parse(raw):[];rows=Array.isArray(parsed)?parsed:[]}catch{}
  const next=[bundle.envelope,...rows].slice(0,250);localStorage.setItem(REVIEW_LIBRARY_KEY,JSON.stringify(next));setSaved(true);
 }
 ...
 return <section style={{marginTop:18,border:"1px solid rgba(92,187,126,.28)",borderRadius:16,padding:16,background:"rgba(7,20,12,.78)"}}>
  ...
  <p style={{margin:"0 0 12px",opacity:.75,fontSize:12}}>Esta mão pode alimentar PLAYER DNA, evolução, treino e DAILY PULSE sem alterar a matemática da análise.</p>
  ...
  <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:14}}><button type="button" className="primary" onClick={saveEvolution}>{saved?"ENVIADO PARA EVOLUÇÃO":"ENVIAR PARA EVOLUÇÃO"}</button><button type="button" onClick={download}>EXPORTAR STACKUP</button></div>
 </section>
}
