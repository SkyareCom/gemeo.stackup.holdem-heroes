...
export default function PlayerDnaWorkspace(){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(raw){
        const parsed=JSON.parse(raw) as Partial<DnaLibrary>;
        const reports=(parsed.reports??[]).map((report:any)=>migrateReport(report));
        const active=parsed.active?migrateSession(parsed.active,reports.length+1):null;
        next={active,reports};
      }else{
        const legacy=localStorage.getItem(LEGACY_STORAGE_KEY);
        if(legacy){const parsed=JSON.parse(legacy);next={active:parsed.finished?null:migrateSession(parsed,1),reports:[]}}
      }
    }catch{}
  ...
  useEffect(()=>{
    ...
    const snapshot:SavedDnaSession={mode,target,index,answers,finished:false,sessionSeed,updatedAt:Date.now(),name:sessionName||`ANÁLISE · ${mode}`,createdAt:createdAt||Date.now()};
    setLibrary(prev=>({...prev,active:snapshot}));
  },[hydrated,mode,target,index,answers,finished,sessionSeed,sessionName,createdAt]);

  useEffect(()=>{
    if(!hydrated||target===null||!finished||answers.length<target)return;
    setLibrary(prev=>{
      const id=String(sessionSeed);if(prev.reports.some(report=>report.id===id))return{...prev,active:null};
      const completedAt=Date.now();
      const development=buildPlayerDevelopmentReport(mode,session,answers,result);
      const report:DnaReport={mode,target,index,answers,finished:true,sessionSeed,updatedAt:completedAt,id,completedAt,name:sessionName||`ANÁLISE ${prev.reports.length+1} · ${mode}`,createdAt:createdAt||completedAt,result:{label:result.label,scores:{...result.scores}},development};
      return{active:null,reports:[report,...prev.reports]};
    });
  },[hydrated,target,finished,answers,mode,index,sessionSeed,result,session,sessionName,createdAt]);
  ...
  function resetAll(){try{localStorage.removeItem(STORAGE_KEY);localStorage.removeItem(OLD_STORAGE_KEY);localStorage.removeItem(LEGACY_STORAGE_KEY)}catch{}setLibrary(emptyLibrary);setSelectedReportId(null);setHistoryOpen(false);setEditingId(null);setTarget(null);setIndex(0);setAnswers([]);setFinished(false);setSessionSeed(1)}
  ...
  if(selectedReport){const previous=library.reports.find(r=>r.id!==selectedReport.id&&r.mode===selectedReport.mode&&r.completedAt<selectedReport.completedAt)??null;return <ReportView report={selectedReport} previous={previous} onBack={()=>{setSelectedReportId(null);setHistoryOpen(true)}} onRename={()=>beginRename(selectedReport)} onChecklist={updateChecklist}/>}
  ...
  if(target===null)return <div className={styles.setup}>
    ...
    {library.active&&<div className={styles.block}>
      <div className={styles.progressHead}><span>ANÁLISE SALVA · {library.active.name}</span><strong>{Math.round(Math.min(library.active.answers.length,library.active.target)/library.active.target*100)}%</strong></div>
      ...
      <small>{library.active.mode} · {library.active.answers.length} / {library.active.target} SPOTS</small>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8}}><button type="button" className="primary" onClick={continueSaved}>CONTINUAR ANÁLISE</button></div>
    </div>}
...
function ReportView({report,previous,onBack,onRename,onChecklist}:{report:DnaReport;previous:DnaReport|null;onBack:()=>void;onRename:()=>void;onChecklist:(reportId:string,itemId:string,patch:Partial<DnaChecklistItem>)=>void}){
  ...
  function download(){const html=reportToStandaloneHtml(report.name,report.mode,report.completedAt,report.result.scores,report.development);const blob=new Blob([html],{type:"text/html;charset=utf-8"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`${safeName(report.name)}.html`;a.click();URL.revokeObjectURL(url)}
  function downloadJson(){const blob=new Blob([JSON.stringify(report,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`${safeName(report.name)}.json`;a.click();URL.revokeObjectURL(url)}
  ...
  return <div className={styles.result}>
    <div><span className="tag">PLAYER DEVELOPMENT REPORT · {report.mode}</span><h3>{report.name}</h3><p>{new Date(report.completedAt).toLocaleString("pt-BR")} · {report.answers.length} / {report.target} SPOTS</p></div>
    <div className={styles.resultGrid}><Metric label="AGRESSÃO" value={`${report.result.scores.aggression}%`}/><Metric label="DISCIPLINA" value={`${report.result.scores.discipline}%`}/><Metric label="PRESSÃO" value={`${report.result.scores.pressure}%`}/><Metric label="PASSIVIDADE" value={`${report.result.scores.passivity}%`}/></div>
    ...
    {deltas&&<ReportSection title="EVOLUÇÃO DESDE O RELATÓRIO ANTERIOR"><div className={styles.resultGrid}><Metric label="AGRESSÃO" value={delta(deltas.aggression)}/><Metric label="DISCIPLINA" value={delta(deltas.discipline)}/><Metric label="PRESSÃO" value={delta(deltas.pressure)}/><Metric label="PASSIVIDADE" value={delta(deltas.passivity)}/></div></ReportSection>}
    <ReportSection title="COMO O RESULTADO FOI OBTIDO">{report.development.methodology.map((m,i)=><p key={m}>{i+1}. {m}</p>)}</ReportSection>
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button type="button" className="primary" onClick={onBack}>VOLTAR AO HISTÓRICO</button><button type="button" className={styles.modeButton} onClick={onRename}><strong>EDITAR NOME</strong></button><button type="button" className={styles.modeButton} onClick={download}><strong>BAIXAR RELATÓRIO</strong></button><button type="button" className={styles.modeButton} onClick={downloadJson}><strong>BAIXAR DADOS</strong></button><button type="button" className={styles.modeButton} onClick={whatsapp}><strong>ENVIAR PLANO NO WHATSAPP</strong></button></div>
  </div>
}
