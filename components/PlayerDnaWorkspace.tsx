...
export default function PlayerDnaWorkspace(){
  ...
  if(target===null)return <div className={styles.setup}>
    ...
    <div className={styles.modeGrid}><button type="button" aria-pressed={mode==="CASH"} className={`${styles.modeButton} ${mode==="CASH"?styles.modeButtonActive:""}`} onClick={()=>setMode("CASH")}><strong>CASH</strong><small>JOGO A DINHEIRO</small></button><button type="button" aria-pressed={mode==="TORNEIO"} className={`${styles.modeButton} ${mode==="TORNEIO"?styles.modeButtonActive:""}`} onClick={()=>setMode("TORNEIO")}><strong>TORNEIO</strong><small>STACKS, ANTES E ICM</small></button></div>
    ...
    <div style={{border:"1px solid rgba(92,187,126,.22)",borderRadius:16,padding:16,background:"rgba(5,20,12,.7)",display:"grid",gap:12}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><div className="eyebrow">RELATÓRIOS E HISTÓRICO</div><strong style={{display:"block",marginTop:4}}>{library.reports.length} {library.reports.length===1?"ANÁLISE CONCLUÍDA":"ANÁLISES CONCLUÍDAS"}</strong><small style={{opacity:.65}}>ACESSE RESULTADOS FINAIS, DATAS E NOMES DAS SUAS ANÁLISES.</small></div><button type="button" aria-expanded={historyOpen} className={styles.modeButton} style={{minHeight:44}} onClick={()=>setHistoryOpen(v=>!v)}><strong>{historyOpen?"FECHAR":"ABRIR HISTÓRICO"}</strong></button></div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end"}}><button type="button" className={styles.modeButton} style={{minHeight:44}} onClick={resetAll}><strong>ZERAR TUDO</strong></button></div></div>
  </div>;
