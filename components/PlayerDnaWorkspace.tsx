...
export default function PlayerDnaWorkspace(){
  ...
  function answer(action:PlayerAction){
    ...
    setAnswers(next);
    const practicalTarget=Math.min(target,pool.length);
    if(next.length>=practicalTarget){setFinished(true);return}
    setIndex(v=>v+1);
  }

  if(target===null)return <div className={styles.setup}>
    <p>Escolha a modalidade e a profundidade do teste.</p>
  ...
  return <div className={styles.session}>
    <div className={styles.progressHead}><span>SPOT {answers.length+1} / {practicalTarget}</span><strong>{Math.round(((answers.length+1)/practicalTarget)*100)}%</strong></div>
    <div className={styles.track}><i style={{width:`${((answers.length+1)/practicalTarget)*100}%`}}/></div>
...
function Cell({label,children}:{label:string;children:React.ReactNode}){return <div className={styles.cell}><span className={styles.label}>{label}</span>{children}</div>}
