...
export default function PlayerDnaAnalysisField(){
  useEffect(()=>{
    ...
    const apply=()=>{
      ...
      const anchor=nativeSizingContainer??nativeActionContainer??session.querySelector<HTMLElement>('[aria-label="MESA DE POKER ANIMADA PLAYER DNA"]');if(!anchor)return;
      let structureChanged=false;let fixed=session.querySelector<HTMLElement>("[data-fixed-player-actions]");
      if(!fixed){structureChanged=true;fixed=document.createElement("div");fixed.dataset.fixedPlayerActions="true";fixed.className="fixed-player-actions";anchor.insertAdjacentElement("afterend",fixed);fixed.addEventListener("click",event=>{const target=(event.target as HTMLElement).closest<HTMLButtonElement>("button[data-base]");if(!target||target.disabled)return;const base=target.dataset.base??"";const size=target.dataset.size??"";const liveActions=[...session.querySelectorAll<HTMLButtonElement>('button[aria-pressed]')].filter(btn=>ACTION_NAMES.includes((btn.textContent??"").trim())&&!btn.closest("[data-fixed-player-actions]"));const baseButton=liveActions.find(btn=>(btn.textContent??"").trim()===base);if(!baseButton)return;if(baseButton.getAttribute("aria-pressed")!=="true")baseButton.click();if(size){window.setTimeout(()=>{const sizeButton=[...session.querySelectorAll<HTMLButtonElement>('button[aria-pressed]')].find(btn=>(btn.textContent??"").trim()===size&&!btn.closest("[data-fixed-player-actions]"));sizeButton?.click()},100)}})}
      let comment=session.querySelector<HTMLElement>("[data-player-comment-card]");if(!comment){structureChanged=true;comment=document.createElement("div");comment.dataset.playerCommentCard="true";comment.className="player-comment-card awaiting-comment";fixed.insertAdjacentElement("afterend",comment)}
    ...
    const observer=new MutationObserver(apply);observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:["aria-pressed","aria-disabled","class"]});
    const timer=window.setInterval(apply,120);return()=>{observer.disconnect();window.clearInterval(timer);style.remove()};
  },[]);
  return null;
}
