export const STACKUP_EXCHANGE_VERSION="1.2.0" as const;
...
export type StackupDeliveryChannel="WHATSAPP"|"PUSH"|"EMAIL"|"TELEGRAM"|"IN_APP";
export type StackupPulseKind="SPOT_RAPIDO"|"QUIZ"|"VOCE_SABIA"|"CONCEITO"|"REVISAO_LEAK";
...
export type StackupDailyPulsePreference={
 id:string;player:StackupPlayerRef;enabled:boolean;channels:StackupDeliveryChannel[];deliveriesPerDay:1|2|3;preferredTimes?:string[];quietHours?:{start:string;end:string}|null;timezone?:string|null;difficultyMin?:number|null;difficultyMax?:number|null;modeMix?:StackupGameMode[];kinds:StackupPulseKind[];prioritizeLeaks:boolean;spacedRepetition:boolean;maxSameTagPerDay?:number|null;createdAt:number;updatedAt:number;
};
export type StackupDailyPulseOption={id:string;label:string};
export type StackupDailyPulseMessage={
 id:string;player:StackupPlayerRef;kind:StackupPulseKind;channel:StackupDeliveryChannel;source:StackupSource;sourceRecordId?:string|null;title:string;body:string;spot?:StackupSpotExchange|null;options?:StackupDailyPulseOption[];correctOptionId?:string|null;explanation?:string|null;conceptTags:string[];leakTags:string[];trainingTags:string[];difficulty?:number|null;scheduledFor:number;sentAt?:number|null;expiresAt?:number|null;requiresResponse:boolean;metadata?:Record<string,unknown>;
};
export type StackupDailyPulseResponse={
 id:string;messageId:string;player:StackupPlayerRef;channel:StackupDeliveryChannel;selectedOptionId?:string|null;textResponse?:string|null;receivedAt:number;decisionMs?:number|null;outcome:StackupDecisionOutcome;confidenceSelfReported?:number|null;metadata?:Record<string,unknown>;
};
export type StackupDailyPulsePerformance={
 id:string;player:StackupPlayerRef;windowStart:number;windowEnd:number;messagesSent:number;responses:number;correct:number;incorrect:number;timeouts:number;responseRate:number;accuracy:number;averageDecisionMs?:number|null;topWeakTags:string[];improvedTags:string[];recommendedNextKinds:StackupPulseKind[];createdAt:number;
};

export type StackupExchangeEnvelope={exchangeFamily?:typeof STACKUP_EXCHANGE_FAMILY;exchangeVersion:string;exportedAt:number;producer:string;producerVersion?:string|null;player:StackupPlayerRef;analyses?:StackupAnalysisExchange[];spots?:StackupSpotExchange[];evidences?:StackupEvidenceExchange[];leaks?:StackupLeakExchange[];prescriptions?:StackupTrainingPrescription[];trainingSessions?:StackupTrainingSession[];trainingResults?:StackupTrainingResult[];evolution?:StackupEvolutionSnapshot[];dailyPulsePreferences?:StackupDailyPulsePreference[];dailyPulseMessages?:StackupDailyPulseMessage[];dailyPulseResponses?:StackupDailyPulseResponse[];dailyPulsePerformance?:StackupDailyPulsePerformance[];metadata?:Record<string,unknown>};
