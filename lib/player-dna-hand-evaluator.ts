...
function facingBet(spot:PlayerDnaSpot){const hero=spot.players.find(p=>p.hero);if(!hero)return null;const previous=spot.players.filter(p=>!p.hero&&p.value>0&&p.action!=="FOLD").map(p=>p.value);if(!previous.length)return null;const call=Math.max(...previous);return{call,pot:spot.pot.main}}
...
export function evaluateHandDecision(spot:PlayerDnaSpot,action:PlayerAction,sizing?:DecisionSizing|null):HandEvaluation{
  ...
  const confidence=Math.max(90,Math.min(95,90+(bestFreq>=60?2:1)+(multiway(spot)?0:1)+(icm(spot)?0:1)));
