...
const PREFLOP_PREMIUM=["AA","KK","QQ","AKS","AKO","JJ","AQS"];
...
function boardState(spot:PlayerDnaSpot){const board=cards(spot.board);const values=[...new Set(board.map(c=>RANK_VALUE[rank(c)]??0))].sort((a,b)=>a-b);const rankCounts=new Map<string,number>(),suitCounts=new Map<string,number>();board.forEach(c=>{rankCounts.set(rank(c),(rankCounts.get(rank(c))??0)+1);suitCounts.set(suit(c),(suitCounts.get(suit(c))??0)+1)});const paired=[...rankCounts.values()].some(n=>n>=2);const fourStraight=fourToStraight(values);const maxSuit=Math.max(0,...suitCounts.values());const twoTone=maxSuit>=2;const monotone=board.length===3&&maxSuit===3;let connected=false;for(let i=1;i<values.length;i++)if(values[i]-values[i-1]<=2)connected=true;const dry=!paired&&!twoTone&&!connected;const topRank=Math.max(0,...values);return{paired,fourStraight,dry,monotone,topRank}}
...
export function profileHand(spot:PlayerDnaSpot):HandProfile{
  ...
  const rankCounts=new Map<string,number>(),suitCounts=new Map<string,number>(),boardRankCounts=new Map<string,number>();
  all.forEach(c=>{rankCounts.set(rank(c),(rankCounts.get(rank(c))??0)+1);suitCounts.set(suit(c),(suitCounts.get(suit(c))??0)+1)});
  board.forEach(c=>boardRankCounts.set(rank(c),(boardRankCounts.get(rank(c))??0)+1));
  ...
  const heroMatchesBoard=hero.filter(c=>(boardRankCounts.get(rank(c))??0)>0);
  const topPair=hero.some(c=>(RANK_VALUE[rank(c)]??0)===boardTop)&&(boardRankCounts.get(rank(hero.find(c=>(RANK_VALUE[rank(c)]??0)===boardTop)??""))??0)===1;
  ...
  const boardPaired=[...boardRankCounts.values()].some(n=>n>=2);
  const boardPairTwoPair=made==="TWO_PAIR"&&boardPaired&&heroMatchesBoard.length===1;
  const monsterMade=["QUADS","FULL_HOUSE","FLUSH","STRAIGHT","TRIPS","TWO_PAIR"].includes(made)&&!boardPairTwoPair;
  ...
  if(spot.street!=="PREFLOP"){
    if(monsterMade||overpair)tier="PREMIUM";
    else if(topPair||boardPairTwoPair||comboDraw)tier="STRONG";
    else if(made==="PAIR"||flushDraw||draw.openEnded)tier="MEDIUM";
  }
  const equityClass:HandProfile["equityClass"]=monsterMade||overpair?"MONSTER":topPair||boardPairTwoPair?"STRONG":comboDraw||flushDraw||draw.openEnded?"DRAW":made==="PAIR"||draw.gutshot?"MARGINAL":"AIR";
...
function preflopBaseRange(position:string,tight:boolean,stack:number){
  ...
  if(stack<=15)return tight?"77+,AJS+,AQO+,KQS":"55+,A9S+,ATO+,KQS,KJS,QJS";
  if(["UTG","UTG1","UTG2"].includes(p))return tight?"88+,AJS+,AQO+,KQS":"77+,ATS+,AJO+,KQS,KJS,QJS";
  if(["MP1","MP2","LJ","HJ"].includes(p))return tight?"77+,ATS+,AQO+,KQS":"66+,A9S+,ATO+,KJS+,QJS,JTS";
  if(p==="CO")return tight?"66+,A9S+,ATO+,KJS+,QJS,JTS":"44+,A5S+,A8O+,K9S+,KTO+,Q9S+,J9S+,T9S";
  if(p==="BTN")return tight?"55+,A7S+,A9O+,KTS+,KQO,QTS+,JTS":"22+,A2S+,A7O+,K7S+,K9O+,Q8S+,QTO+,J8S+,T8S+,98S";
  return tight?"66+,A8S+,ATO+,KTS+,QTS+,JTS":"22+,A2S+,A8O+,K7S+,KTO+,Q8S+,J8S+,T8S+,98S";
}
...
export function solverInspiredVillainRange(spot:PlayerDnaSpot,position:string,action:string){
  ...
  const base=spot.street==="PREFLOP"?preflopBaseRange(position,tight,stack):(tight?(late?"TT+,AQS+,AKO":"JJ+,AKS,AKO"):(late?"66+,A8S+,KTS+,QTS+,JTS,ATO+,KQO":"88+,ATS+,KQS,AQO+"));
...
export function solverNodePolicy(spot:PlayerDnaSpot,position:string,templateAction:string):SolverNodePolicy{
  ...
  }else if(allowed.includes("BET"))base=spot.street==="FLOP"?{BET:92,"ALL-IN":8}:spot.street==="TURN"?{BET:88,"ALL-IN":12}:{BET:84,"ALL-IN":16};
  else if(allowed.includes("RAISE"))base={RAISE:multiway?82:88,"ALL-IN":multiway?18:12};
  else if(allowed.includes("CALL"))base={CALL:multiway?86:78,RAISE:multiway?14:22};
  else base=spot.street==="FLOP"?{CHECK:68,BET:32}:spot.street==="TURN"?{CHECK:73,BET:27}:{CHECK:79,BET:21};
...
export function solverHeroNodeStrategy(spot:PlayerDnaSpot){
  ...
  if(spot.street==="PREFLOP"){
    ...
    if(profile.made==="STRAIGHT"&&board.fourStraight&&spot.street==="RIVER")f={CALL:100};
    else if(profile.made==="STRAIGHT"&&board.fourStraight)f={CALL:94,RAISE:6};
    else if(profile.equityClass==="MONSTER")f={FOLD:1,CALL:18,RAISE:72,"ALL-IN":9};
    else if(spot.street==="FLOP"&&profile.equityClass==="STRONG"&&profile.topPair&&board.dry&&board.topRank<14&&spr>=4&&!profile.flushDraw&&!profile.openEnded&&!profile.gutshot)f={CALL:44,RAISE:56};
    ...
    else if(profile.equityClass==="STRONG")f={FOLD:1,CALL:96,RAISE:3};
    ...
    if(spot.mode==="TORNEIO"&&spot.street==="FLOP"&&ip&&profile.equityClass==="STRONG"&&spr<=3.25&&!profile.flushDraw&&!profile.openEnded&&!profile.gutshot)f={CHECK:87,BET:13};
    ...
    else if(profile.equityClass==="STRONG"&&board.paired&&spot.street==="FLOP")f={CHECK:49,BET:51};
    ...
    else if(profile.equityClass==="DRAW"&&spot.street==="FLOP"&&ip&&board.monotone&&profile.flushDraw)f={CHECK:5,BET:95};
    ...
    else f={CHECK:80,BET:19,"ALL-IN":1};
  }
